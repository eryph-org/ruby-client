...
RSpec.describe 'Zero Configuration Auto-Discovery', :integration do
  describe 'system client discovery' do
    it 'tests system client access based on admin privileges' do
      ...
      ps_can_access = $?.success? && !ps_system_result.strip.empty?
      ...
      if ruby_can_access && ps_can_access
        ...
        fail "PowerShell can access system-client but Ruby cannot - this indicates a Ruby client bug"
      else
  ...
  describe 'local configuration fallback' do
    it 'handles local configuration when zero not available' do
      ...
      if $?.success? && !ps_local_result.strip.empty?
  ...
  describe 'configuration precedence with zero' do
    it 'follows expected precedence when multiple configurations exist' do
      test_configs = ['test-default', 'test-local']
      temp_keys = {}
      
      begin
        # Create test configurations to test precedence
        test_configs.each_with_index do |config_name, index|
          ...
          ps_result = `powershell -Command "
            Get-Content '#{temp_key}' -Raw | New-EryphClientCredentials -Id '#{client_id}' -IdentityEndpoint 'https://#{config_name}.example.com/identity' -Configuration '#{config_name}' | Add-EryphClientConfiguration -Id '#{client_id}' -Name 'Test #{config_name}' -Configuration '#{config_name}' | ConvertTo-Json
          "`
          expect($?.success?).to be_truthy, "Failed to create test config #{config_name}: #{ps_result}"
        ...
        test_configs.each do |config_name|
          ...
          expect($?.success?).to be_truthy, "PowerShell should find #{config_name}"
          
          ps_config = JSON.parse(ps_result)
          ps_config = [ps_config] unless ps_config.is_a?(Array)
          expect(ps_config.first['Id']).not_to be_nil
          
          # Ruby should find same config
          ruby_client = Eryph::Compute::Client.new(config_name)
          ruby_credentials = ruby_client.token_provider.credentials
  ...
  describe 'zero configuration without admin privileges' do
    it 'gracefully handles system client access restrictions' do
      ...
      begin
        ...
        token = ruby_client.token_provider.get_access_token
        ...
        if !$?.success? || ps_zero_check.strip.empty?
        else
          fail "Ruby failed zero config access but PowerShell succeeded - this indicates a Ruby bug"
        end
        
      rescue => e
        # Any other error should be investigated
        fail "Unexpected error accessing zero configuration: #{e.class}: #{e.message}"
      end
    end
  end
end