...
RSpec.describe 'Credential Discovery', :integration do
  ...
  describe 'private key discovery' do
    it 'Ruby discovers same private key that PowerShell configured' do
      ...
      begin
        # Generate a test private key that PowerShell can use
        test_private_key = OpenSSL::PKey::RSA.generate(2048)
        File.write(temp_key, test_private_key.to_pem)

        # PowerShell creates client with the provided private key as default
        ps_cred_result = `powershell -Command "
          Get-Content '#{temp_key.gsub('/', '\\')}' -Raw | New-EryphClientCredentials -Id '#{test_client_id}' -IdentityEndpoint 'https://cred-test.example.com/identity' -Configuration '#{test_config_name}' | Add-EryphClientConfiguration -Id '#{test_client_id}' -Name 'Credential Test Client' -Configuration '#{test_config_name}' -AsDefault | ConvertTo-Json
        "`
        expect($CHILD_STATUS.success?).to be_truthy, "PowerShell failed to create credentials: #{ps_cred_result}"
        ...
        expect($CHILD_STATUS.success?).to be_truthy, 'PowerShell failed to retrieve stored credentials'
        ...
        # Test Ruby discovers same credentials using ClientCredentialsLookup directly
        ...
        token_provider = Eryph::ClientRuntime::TokenProvider.new(ruby_credentials)
        jwt_assertion = token_provider.send(:create_client_assertion)
    ...
    it 'handles credentials in different store than client configuration' do
      ...
      begin
        ...
        ps_sep_result = `powershell -Command "
          ...
          Get-Content '#{temp_key}' -Raw | New-EryphClientCredentials -Id '#{test_client_id}' -IdentityEndpoint 'https://separated.example.com/identity' -Configuration '#{test_config_name}' | Add-EryphClientConfiguration -AsDefault | ConvertTo-Json
        "`
        expect($CHILD_STATUS.success?).to be_truthy, "PowerShell failed separated store setup: #{ps_sep_result}"
        ...
        "`
        expect($CHILD_STATUS.success?).to be_truthy, "PowerShell failed to verify separated credentials, Response: #{ps_verify_creds}"
        ...
        # Test Ruby can handle separated stores using lower-level credential lookup
        environment = Eryph::ClientRuntime::Environment.new
        reader = Eryph::ClientRuntime::ConfigStoresReader.new(environment)
        lookup = Eryph::ClientRuntime::ClientCredentialsLookup.new(reader, test_config_name)
        ruby_credentials = lookup.find_credentials
    ...
    it 'detects invalid or missing private keys' do
      ...
      begin
        ...
        ps_invalid_result = `powershell -Command "
          Get-Content '#{temp_key}' -Raw | New-EryphClientCredentials -Id '#{test_client_id}' -IdentityEndpoint 'https://invalid.example.com/identity' -Configuration '#{test_config_name}' | Add-EryphClientConfiguration -Id '#{test_client_id}' -Name 'Invalid Key Test' -Configuration '#{test_config_name}' -AsDefault | ConvertTo-Json
        "`
        ...
        if $CHILD_STATUS.success?
          ...
          expect do
            ...
            token_provider = Eryph::ClientRuntime::TokenProvider.new(credentials)
