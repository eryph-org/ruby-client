      ...
      it "discovers configurations from #{store_location} store" do
        ...
        begin
          ...
          `powershell -Command "
            ...
            Get-Content '#{temp_key}' -Raw | New-EryphClientCredentials -Id '#{test_client_id}' -IdentityEndpoint 'https://test.example.com/identity' -Configuration '#{test_config_name}' | Add-EryphClientConfiguration -Id '#{test_client_id}' -Name 'Test Store Client' -Configuration '#{test_config_name}';
          "`
          ...
          "`
          expect($?.success?).to be_truthy, "PowerShell failed to read configuration from #{store_location} store"
          ...
          # Test Ruby discovers configuration from same store
    ...
    it 'follows CurrentDirectory > User > System precedence' do
      ...
      begin
        ...
        `powershell -Command "
          ...
          Add-EryphClientConfiguration -Id '#{test_client_id}-user' -Name 'User Store Client' -Credentials $creds -Configuration '#{test_config_name}';
        "`
        ...
        `powershell -Command "
          ...
          Add-EryphClientConfiguration -Id '#{test_client_id}-current' -Name 'Current Dir Client' -Credentials $creds -Configuration '#{test_config_name}';
        "`
        
        # Ruby should find CurrentDirectory version (higher precedence)
        ruby_client = Eryph::Compute::Client.new(test_config_name)
        ruby_credentials = ruby_client.token_provider.credentials
