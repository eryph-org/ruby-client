  ...
  describe 'multiple clients in configuration' do
    it 'Ruby and PowerShell handle multiple clients correctly' do
      ...
      begin
        ...
        "`
        expect($CHILD_STATUS.success?).to be_truthy, 'PowerShell failed to create multiple clients'
        ...
        expect($CHILD_STATUS.success?).to be_truthy, 'PowerShell failed to list all clients'
        ...
        expect($CHILD_STATUS.success?).to be_truthy, 'PowerShell failed to get default client'
        ...
        # Test Ruby uses same default client selection logic using lower-level credential lookup
  ...
  describe 'client selection by ID' do
    it 'Ruby can select specific client when multiple exist' do
      ...
      begin
        ...
        "`
        expect($CHILD_STATUS.success?).to be_truthy, 'PowerShell failed client selection setup'
        ...
        expect($CHILD_STATUS.success?).to be_truthy, 'PowerShell failed to get target client by ID'
        ...
        # Ruby should use default client, not the target client using lower-level credential lookup
        environment = Eryph::ClientRuntime::Environment.new
        reader = Eryph::ClientRuntime::ConfigStoresReader.new(environment)
        lookup = Eryph::ClientRuntime::ClientCredentialsLookup.new(reader, test_config_name)
        ruby_credentials = lookup.find_credentials
  ...
  describe 'default client behavior' do
    it 'handles configurations with no explicit default client' do
      ...
      begin
        ...
        "`
        expect($CHILD_STATUS.success?).to be_truthy, 'PowerShell failed to create default client'
