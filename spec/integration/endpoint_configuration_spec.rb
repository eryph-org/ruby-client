    ...
    it 'Ruby discovers same endpoint URLs that PowerShell configured' do
      ...
      begin
        ...
        ps_create_result = `powershell -Command "
          Get-Content '#{temp_key}' -Raw | New-EryphClientCredentials -Id '#{test_client_id}' -IdentityEndpoint '#{test_identity_endpoint}' -Configuration '#{test_config_name}' | Add-EryphClientConfiguration -Id '#{test_client_id}' -Name 'Endpoint Test Client' -Configuration '#{test_config_name}' | ConvertTo-Json
        "`
        expect($?.success?).to be_truthy, "PowerShell failed to configure endpoints: #{ps_create_result}"
        ...
        expect($?.success?).to be_truthy, "PowerShell failed to retrieve credentials: #{ps_creds_result}"
        ...
        # Test Ruby discovers same endpoints
  ...
  describe 'endpoint separation' do
    it 'correctly handles identity endpoint configuration' do
      ...
      begin
        ...
        ps_create_result = `powershell -Command "
          Get-Content '#{temp_key}' -Raw | New-EryphClientCredentials -Id '#{test_client_id}' -IdentityEndpoint '#{different_identity}' -Configuration '#{test_config_name}' | Add-EryphClientConfiguration -Id '#{test_client_id}' -Name 'Separated Endpoints Client' -Configuration '#{test_config_name}' | ConvertTo-Json
        "`
        expect($?.success?).to be_truthy, "PowerShell failed to configure identity endpoint: #{ps_create_result}"
        ...
        # Test Ruby handles identity endpoint correctly
        ruby_client = Eryph::Compute::Client.new(test_config_name)
        ruby_credentials = ruby_client.token_provider.credentials
